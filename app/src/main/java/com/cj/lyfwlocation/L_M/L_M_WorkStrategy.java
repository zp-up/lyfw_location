...
import com.cj.lyfwlocation.L_M.job.L_M_WorkService;
...
public class L_M_WorkStrategy implements I_L_M_Strategy {
    ...
    public void stopWork() {
        ...
        if(infos!=null){
            for (JobInfo info:infos){
                if(info.getId() == WorkServiceConfig.jobID ){
    ...
    private void initScheduler(){
        ...
        JobInfo.Builder jBuilder = new JobInfo.Builder(WorkServiceConfig.jobID,new ComponentName(mContext.getPackageName(), L_M_WorkService.class.getName()));
        //任务执行间隔
        jBuilder.setPeriodic(WorkServiceConfig.period);
